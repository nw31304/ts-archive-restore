...
class AlreadyArchivedOrDoesNotExistError extends Error {
}
exports.AlreadyArchivedOrDoesNotExistError = AlreadyArchivedOrDoesNotExistError;
/**
  ...
  */
let reset_report_s3_location_sql = squel.update()
...
/**
  ...
  */
let reset_analysis_s3_location_sql = squel.update()
...
/**
  ...
  */
let validate_analysis_archived_sql = squel.select()
...
/**
  ...
  */
let validate_report_present_sql = squel.select()
...
/**
  ...
  @param{number} $1 - analysis id
  */
let analysis_s3_location_sql = squel.select()
    .from(`${config.db_connection.schema}.analysis`)
    .field("archive_location")
    .where(squel.expr()
    .and("archive_location is not null")
...
/**
  ...
  @param{number} $1 - report id
  */
let report_s3_location_sql = squel.select()
    .from(`${config.db_connection.schema}.report`)
    .field("archive_location")
    .where(squel.expr()
    .and("archive_location is not null")
...
/**
  ...
  @param{number} $1 - report id
  */
let analyses_by_report_sql = squel.select()
...
/**
  ...
  @param{number} $1 - analysis id
  */
let reports_by_analysis_sql = squel.select()
    .from(`${config.db_connection.schema}.analysis_report`, "j")
    .field("report")
    .distinct()
...
let connectStr = `postgresql://${config.db_connection.user}:${config.db_connection.password}@${config.db_connection.host}:${config.db_connection.port}/${config.db_connection.database}`;
/**
 ...
 */
function get_s3_location(db, id, ps) {
 ...
 * @return{Promise<string>} Promise resolved with a string containing the S3 object name
 */
let get_report_s3_location = _.partialRight(get_s3_location, report_s3_location_sql);
/**
 ...
 * @return{Promise<string>} Promise resolved with a string containing the S3 object name
 */
let get_analysis_s3_location = _.partialRight(get_s3_location, analysis_s3_location_sql);
/**
...
function restoreReports(ids) {
    let ps = ids.map((id) => restoreReport(id));
 ...
 */
function archiveAnalyses(ids) {
    let ps = ids.map((id) => archiveAnalysis(id));
    return Promise.all(ps);
}
/**
 ...
 */
function dumpAnalysis(a, fn) {
    let objs = `-t "${config.db_connection.schema}.analysis_${a}_*"`;
    return dump(fn, objs);
}
/**
 ...
 */
function dumpReport(r, fn) {
    let objs = `-t "${config.db_connection.schema}.segment_${r}" -t "${config.db_connection.schema}.stats_${r}_*"`;
    return dump(fn, objs);
}
/**
 ...
 *                  target file.
 */
function download(fn, bucket, key) {
    return new Promise((resolve, reject) => {
 ...
 */
function uploadAnalysisDump(a, fn) {
 ...
 */
function uploadReportDump(r, fn) {
...
//# sourceMappingURL=ts-archive-restore.js.map